...
CLEANUP_DONE="0"
CHROME_USER_DATA_DIR="${CHROME_USER_DATA_DIR:-$HOME/.config/google-chrome-automation}"
...
echo -e "${BLUE}   Browser: Google Chrome Debug (9222)${NC}"
echo ""
...
kill_chrome_debug_9222() {
  ...
  if is_port_busy 9222; then
    echo -e "${YELLOW}⚠️  Porta 9222 ainda ocupada. Liberando...${NC}"
  ...
  (
    ...
    while true; do
      ...
      if pgrep -af "google-chrome.*remote-debugging-port=9222" >/dev/null 2>&1; then
        {
          echo "[$(date '+%Y-%m-%d %H:%M:%S')] WARN: CDP 9222 sem resposta, mas Chrome Debug ainda esta em execucao. Aguardando sem reiniciar."
        } >> "$CHROME_WATCHDOG_LOG"
        continue
      fi
      ...
      restarting=1
      {
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] WARN: Chrome Debug offline (9222). Tentando relancar..."
      } >> "$CHROME_WATCHDOG_LOG"
...
if wait_http "http://127.0.0.1:9222/json/version" 2; then
